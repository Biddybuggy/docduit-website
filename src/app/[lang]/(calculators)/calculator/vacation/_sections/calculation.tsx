...
export default function CalculationSection({
  ...
  const [where, setWhere] = useState<string>('Bali');
  ...
  const handleSubmit = () => {
    onSubmit({ term, price, budget, label: where });
  };
      ...
      <div className='w-full flex flex-col gap-8 items-center justify-center'>
        <div className='w-full flex flex-col gap-4'>
          <div className='flex flex-col gap-2'>
            <p className='text-lg font-normal'>{whereQuestion}</p>
            <Input
              onChange={(e) => setWhere(e.target.value)}
              value={where}
              className='ring-0 border-0 !bg-transparent text-center !font-semibold !text-4xl text-docduit-blue underline decoration-1 underline-offset-4'
            />
          </div>
          <div className='flex flex-col gap-2'>
